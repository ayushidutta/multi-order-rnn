...
class MultilabelEvaluate:
    def __init__(self,
        ...
        self.nCorrect_row = self.nCorrect_row + np.finfo(np.float).eps
        self.nGround_row = self.nGround_row + np.finfo(np.float).eps
        self.nPredict_row = self.nPredict_row + np.finfo(np.float).eps
    ...
    def calc_f1(self):
        ...
        self.results['f1'] = np.mean(f1_row)
...
def evaluate_per_row_col_metrics(ground_annot, prediction_scores, predicted_annot, topK, row_wise):
